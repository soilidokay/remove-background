...
def calculate_bounding_box_around_longest_straightest_vertical_lines_laplacian(image_path, output_path=None, min_line_length=50, max_line_gap=30, angle_tolerance=10, gaussian_ksize=(7, 7), laplacian_ksize=5, laplacian_threshold=100):
    """
    ...
    để bằng với chiều cao của ảnh gốc, sử dụng Laplacian để dò cạnh.
    ...
    Args:
        ...
        laplacian_ksize (int): Kích thước kernel cho Laplacian (phải là số lẻ, ví dụ: 1, 3, 5).
        laplacian_threshold (int): Ngưỡng để nhị phân hóa ảnh cạnh Laplacian.
    ...
    # Đây là bước tiền xử lý quan trọng cho Laplacian
    blurred = cv2.GaussianBlur(gray, gaussian_ksize, 0)
    cv2.imshow(
        f"Giai đoạn 3: Ảnh đã làm mượt (Gaussian Blur {gaussian_ksize})", blurred)
    cv2.waitKey(0)

    # Giai đoạn 4: Dò cạnh bằng Laplacian
    # Áp dụng Laplacian lên ảnh đã làm mượt
    laplacian = cv2.Laplacian(blurred, cv2.CV_64F, ksize=laplacian_ksize)

    # Chuyển đổi kết quả Laplacian sang kiểu 8-bit không dấu để hiển thị
    # Giá trị âm sẽ trở thành 0, giá trị dương sẽ là cạnh
    laplacian_abs = np.uint8(np.absolute(laplacian))
    cv2.imshow(
        f"Giai đoạn 4a: Laplacian (ksize={laplacian_ksize})", laplacian_abs)
    cv2.waitKey(0)

    # Ngưỡng hóa kết quả Laplacian để tạo ảnh cạnh nhị phân
    # Các giá trị lớn hơn ngưỡng sẽ trở thành 255 (cạnh), còn lại là 0
    _, edges = cv2.threshold(
        laplacian_abs, laplacian_threshold, 255, cv2.THRESH_BINARY)
    cv2.imshow(
        f"Giai đoạn 4b: Cạnh (Laplacian) - Ngưỡng {laplacian_threshold}", edges)
    cv2.waitKey(0)
    ...
    if lines is not None:
        for line in lines:
            ...
            if angle_deviation <= angle_tolerance:
                ...
                candidate_vertical_lines.append(
                    (x1, y1, x2, y2, length, angle_deviation))
                # Vẽ đường thẳng dọc màu xanh lá
                cv2.line(lines_drawn_img, (x1, y1), (x2, y2), (0, 255, 0), 2)
            else:
                # Vẽ đường thẳng không dọc màu đỏ
                cv2.line(lines_drawn_img, (x1, y1), (x2, y2), (0, 0, 255), 1)

    cv2.imshow(
        "Giai đoạn 5: Đường thẳng đã phát hiện (Xanh: dọc, Đỏ: khác)", lines_drawn_img)
    cv2.waitKey(0)
    ...
    # Score sẽ là length - angle_deviation * weight (ưu tiên dài hơn, ít lệch góc hơn)
    best_left_score = -1.0
        ...
        for line_data in candidate_vertical_lines:
            ...
            if mid_x_line < width / 2:  # Đường thẳng ở nửa trái ảnh
                if current_score > best_left_score:
                    ...
                    # Lấy tọa độ x nhỏ nhất của đường này
                    leftmost_best_line_x = min(x1, x2)
            else:  # Đường thẳng ở nửa phải ảnh
                if current_score > best_right_score:
                    best_right_score = current_score
                    # Lấy tọa độ x lớn nhất của đường này
                    rightmost_best_line_x = max(x1, x2)

        print(
            f"X của đường thẳng dọc dài nhất & thẳng nhất bên trái: {leftmost_best_line_x}")
        print(
            f"X của đường thẳng dọc dài nhất & thẳng nhất bên phải: {rightmost_best_line_x}")
    ...
    result_img = img.copy()
    cv2.rectangle(result_img, (final_x_min, final_y_min),
                  (final_x_max, final_y_max), (255, 0, 0), 2)
    cv2.imshow(
        "Giai đoạn 8: Ảnh với khung bao cuối cùng (Chiều cao ảnh gốc)", result_img)
    cv2.waitKey(0)
...
# Đặt các tham số mới cho Laplacian
# Lưu ý: laplacian_ksize và laplacian_threshold cần được điều chỉnh tùy theo ảnh của bạn
# count = 54 # Giữ lại nếu bạn muốn chạy vòng lặp
# for i in range(48,54): # Giữ lại nếu bạn muốn chạy vòng lặp
#     calculate_bounding_box_around_longest_straightest_vertical_lines_laplacian(f'images/{i}.png', f'outputs_laplacian/{i}.jpg',
#         gaussian_ksize=(5, 5),       # Kích thước kernel Gaussian
#         laplacian_ksize=3,           # Kích thước kernel Laplacian (thường là 1, 3, 5)
#         laplacian_threshold=50       # Ngưỡng để nhị phân hóa cạnh Laplacian
#     )
count = 54
for i in range(48, 54):
    # Ví dụ gọi một lần với ảnh cụ thể
    calculate_bounding_box_around_longest_straightest_vertical_lines_laplacian(
        f'images/{i}.png',
        'output_cropped_laplacian_edges.jpg',
        gaussian_ksize=(5, 5),       # Kích thước kernel Gaussian
        # Kích thước kernel Laplacian (thường là 1, 3, 5)
        laplacian_ksize=3,
        laplacian_threshold=50       # Ngưỡng để nhị phân hóa cạnh Laplacian
    )
